...
function GM:HUDDrawScoreBoard()
	...
	local colWidth = 75
	local upByName = "ptown2"
	local gmName = "Suicide Barrels"
	...
	local BWins = yOffset - 12
	BWins = BWins + txHeight + 2
	BOffset = (ScrW() - boardWidth) / 3.65
	
	local HWins = yOffset - 30
	HWins = HWins + txHeight + 2
	HOffset = (ScrW() - boardWidth) / 3.65
	
	-- surface.SetTextColor( 255, 255, 255, 255 )
	-- surface.SetFont( "RoundFont" )
	-- surface.SetTextPos(BOffset + (boardWidth / 2) - (txWidth/2), BWins)
	-- surface.DrawText( "Total Barrel Wins: " .. round.BarrelWins )
	
	-- surface.SetTextColor( 255, 255, 255, 255 )
	-- surface.SetFont( "RoundFont" )
	-- surface.SetTextPos(HOffset + (boardWidth / 2) - (txWidth/2), HWins)
	-- surface.DrawText( "Total Human Wins: " .. round.HumanWins )
	
	-- local Rty = yOffset - 48
	-- Rty = Rty + txHeight + 2
	-- xRtOffset = (ScrW() - boardWidth) / 3.65
	
	-- surface.SetTextColor( 255, 255, 255, 255 )
	-- surface.SetFont( "RoundFont" )
	-- surface.SetTextPos(xRtOffset + (boardWidth / 2) - (txWidth/2), Rty)
	-- if currentRound == 0 then
		-- surface.DrawText( "Round Not Started" )
	-- else 
		-- surface.DrawText( "Round " ..currentRound .. " - " ..  roundtime )
	-- end
	...
	surface.DrawText( "and Updated by Zephruz" )
